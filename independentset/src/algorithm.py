...
def R3(graph, depth=-1):
    R3.nbr_calls += 1
    depth += 1
    debug("R3({:s})".format(graph.mates.keys()), depth)
    if graph.is_empty():
        debug("No nodes in input, returning 0", depth)
        return 0

    if graph.is_disconnected():
        return graph.size()
    ...
    graph.remove_node(graph.neighbourhood(max_deg_node))
    debug("a = Recursing on G[V-N(maxdegnode)]", depth)
    mis_a = 1 + R3(graph, depth)
    graph.rewind_version()

    graph.new_version()
    graph.remove_node([max_deg_node])
    debug("b = Recursing on G[V-maxdegnode]", depth)
    mis_b = R3(graph, depth)
    graph.rewind_version()

    debug("mis_a = {:d}, mis_b = {:d}".format(mis_a, mis_b), depth)
    return max(mis_a, mis_b)

def _record_mis(algo, new_mis):
    if new_mis > algo.max_mis_found:
        algo.max_mis_found = new_mis

def R4(graph, depth=-1):
    if depth == -1:
        R4.max_mis_found = 0
    R4.nbr_calls += 1
    depth += 1
    debug("R4({:s})".format(graph.mates.keys()), depth)
    if graph.is_empty():
        debug("No nodes in input, returning 0", depth)
        _record_mis(R4, 0)
        return 0

    if graph.size() < R4.max_mis_found:
        return -1
    ...
    if unconnected_node is not None:
        ...
        graph.new_version()
        graph.remove_node([unconnected_node])
        mis_size = 1 + R4(graph, depth)
        graph.rewind_version();
        _record_mis(R4, mis_size)
        return mis_size
    ...
    graph.remove_node(graph.neighbourhood(max_deg_node))
    debug("a = Recursing on G[V-N(maxdegnode)]", depth)
    mis_a = 1 + R4(graph, depth)
    graph.rewind_version()

    graph.new_version()
    graph.remove_node([max_deg_node])
    debug("b = Recursing on G[V-maxdegnode]", depth)
    mis_b = R4(graph, depth)
    graph.rewind_version()
